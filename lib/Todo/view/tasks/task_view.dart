...
class _TaskViewState extends State<TaskView> {
  ...
  String showTime(DateTime? time) {
    ...
    if (widget.task?.createdAtTime == null) {
      if (time == null) {
        //转为local本地时间
        return DateFormat('hh:mm a', 'zh').format(DateTime.now()).toString();
      } else {
        return DateFormat('hh:mm a', 'zh').format(time).toString();
      }
    } else {
      return DateFormat('hh:mm a', 'zh')
  ...
  String showDate(DateTime? date) {
    ...
    if (widget.task?.createdAtDate == null) {
      if (date == null) {
        return DateFormat('yyyy-MM-dd', 'zh').format(DateTime.now()).toString();
      } else {
        return DateFormat('yyyy-MM-dd', 'zh').format(date).toString();
      }
    } else {
      return DateFormat('yyyy-MM-dd', 'zh')
    ...
    if (widget.taskControllerForTitle?.text != null &&
      ...
      if (title != null && subtitle != null) {
        var task = Task.create(
          title: title,
          createdAtTime: time?.toLocal(),
          createdAtDate: date?.toLocal(),
          subtitle: subtitle,
        );
